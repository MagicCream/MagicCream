...
import urllib
import socket
import parseMetadata, parseQuery, parseQuery1_1, services, parseEndpoints
import Tree
...
def decomposeQuery(endpointList, query, decomposition, contact):
    ...
    return decomposeUnionBlock(query.body, endpointList, genPred, prefixes, decomposition, contact)
...
def potentialStarC(triple, triples):
    ...
    for t0 in triples:
        ...
        if (((not triple.subject.constant) and (not t0.subject.constant) and (triple.subject.name == t0.subject.name))
                or
                ((not triple.subject.constant) and (not t0.theobject.constant) and (
                        triple.subject.name == t0.theobject.name))
                or
                ((not triple.theobject.constant) and (not t0.subject.constant) and (
                        triple.theobject.name == t0.subject.name))
                or
                ((not triple.theobject.constant) and (not t0.theobject.constant) and (
                        triple.theobject.name == t0.theobject.name))):
            ps.append(t0)
    return ps
...
def assignEndpointS(tl, l, genPred, prefixes, c):
    ...
    for sg in tl:
        ...
        sn = sg.subject.name
        isLink = (getUri(sg.predicate, prefixes) in ['<http://www.w3.org/2002/07/owl#sameAs>',
...
def selectCurrentBest(options, triple, qcl, ps, genPred, c):
    ...
    if not added or not (getUri(triple.predicate, ps) in genPred or not triple.predicate.constant):
        for ep in options:
            ...
            if not (
                    triple.subject.constant or triple.predicate.constant or triple.theobject.constant) and not ep in currentOptions:
...
def decomposeTest(file):
    endpointList = None
    if file:
        try:
            with open(file) as efile:
                endpointList = parseMetadata.parse(efile)
        except :
            pass
    print(endpointList)
    return endpointList
...
def makePlan(qString, mFile=None, decomposition=None, plan=None, contact=None):
    q = decomposeMetadata(qString, mFile, decomposition, contact)
    if (q == None):
        return None
